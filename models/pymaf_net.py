...
class Regressor(nn.Module):
    ...
    def forward_init(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=1, J_regressor=None):
        ...
        output = {
            ...
            'kp_2d'  : pred_keypoints_2d,
            'kp_3d'  : pred_joints,
            'smpl_kp_3d' : pred_smpl_joints, '''smpl joint'''
            'rotmat' : pred_rotmat,
            'pred_cam': pred_cam,
            'pred_shape': pred_shape,
            'pred_pose': pred_pose,
    ...
    def forward(self, x, J_regressor=None):
        ...
        for rf_i in range(cfg.MODEL.PyMAF.N_ITER):
            ...
            if rf_i == 0:
                ...
                # TODO: use a more sparse SMPL implementation (with 431 vertices) for acceleration #TODO the simplification
...
def pymaf_net(smpl_mean_params, pretrained=True):
    ...
    '''get the adjust smpl model (adjust the model pose and simplification)'''
    model = PyMAF(smpl_mean_params, pretrained)
    return model


def skeleton_extract(batchNumber, smpl_joints, smpl_vertices):

    for i in range(batchNumber):
        for v in range(smpl_vertices.size(dim=1)):
            """get the smpl first joint"""
            temp_joint = smpl_joints[i, 0, :] # batch_th , joint number, 3d(x,y,z)
            tempDist = torch.cdist(smpl_joints[i, v, :], smpl_joints[i, 0, :], p=2)

            for j in range(smpl_vertices.size(dim=1)):
                if(tempDist > torch.cdist(smpl_joints[i, v, :], smpl_joints[i, j, :], p=2)):
                    temp_joint = j[i, j, :]  # batch_th , joint number, 3d(x,y,z)
                    tempDist = torch.cdist(smpl_joints[i, v, :], smpl_joints[i, j, :], p=2)

        smpl_vertices[i, v, :] += torch.sub(temp_joint, smpl_joints[i, v, :]) * 0.5
